...
// Centro ya conectado al backend real de Laravel en esta iteración.
...
export function LoginPage({ platform = false }: { platform?: boolean }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      if (await login(email, password, platform ? 'PLATFORM' : 'CENTER', center?.id)) {
        // Intento en paralelo contra el backend real (no bloquea la navegación).
        // Fallará para las cuentas demo que solo existen en el mock, y eso es
        // esperado: solo las cuentas creadas vía /crear-cuenta existen en Postgres.
        if (!platform && center?.slug === BACKEND_READY_SLUG) {
          sanctum.login(email, password)
            .then(() => toast.success('Sesión también iniciada en el backend real (Laravel).'))
            .catch((err) => console.warn('Backend real: no fue posible iniciar sesión con esta cuenta.', err))
        }
        navigate(platform ? '/plataforma' : base)
      } else {
        setError(platform ? 'Las credenciales no corresponden a una cuenta de plataforma.' : `El correo o la contraseña no corresponden a ${center?.name}.`)
      }
    } catch {
