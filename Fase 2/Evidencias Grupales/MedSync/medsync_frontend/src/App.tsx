...
import {
  ...
  ReschedulePage,
} from "@/pages/appointment-detail-page";
...
function PlatformProtectedRoute() {
  ...
  return !user ? (
    <Navigate to="/plataforma/acceso" replace />
  ) : user.role === "SUPER_ADMIN" ? (
    <Outlet />
  ) : (
    <Navigate to={organization ? centerBase(organization.slug) : "/"} replace />
  );
}
function ResetTenantSession() {
  ...
  return (
    <main className="grid min-h-screen place-items-center">
      <p>Abriendo el portal del centro…</p>
    </main>
  );
}
      ...
      <main className="grid min-h-screen place-items-center text-center">
        <div>
          ...
          <p className="mt-2 text-muted-foreground">
            Revisa la dirección entregada por tu centro médico.
          </p>
        </div>
      </main>
    );
  if (!user)
    return <Navigate to={`${centerBase(center.slug)}/ingresar`} replace />;
...
function RoleRoute({ allowed }: { allowed: Role[] }) {
  ...
  return user && allowed.includes(user.role) ? (
    ...
    <Navigate to={centerPath()} replace />
  );
}
function HomePage() {
  const { user } = useClinic();
...
export default function App() {
  return (
    <Routes>
      ...
      <Route
        path="/centro/:centerSlug/crear-cuenta"
      ...
      <Route path="/centro/:centerSlug" element={<CenterProtectedRoute />}>
        <Route element={<AppLayout />}>
          ...
          <Route
            element={
              <RoleRoute
                allowed={["RECEPCIONISTA", "PROFESIONAL", "PACIENTE"]}
              />
            }
          >
            <Route path="citas" element={<AppointmentsPage />} />
            <Route path="citas/:id" element={<AppointmentHistoryPage />} />
          </Route>
          <Route
            element={<RoleRoute allowed={["RECEPCIONISTA", "PACIENTE"]} />}
          >
            <Route path="citas/:id/reprogramar" element={<ReschedulePage />} />
            <Route path="reservar" element={<BookingPage />} />
          </Route>
          <Route element={<RoleRoute allowed={["RECEPCIONISTA"]} />}>
          ...
          <Route
            element={
              <RoleRoute
                allowed={["ADMIN", "RECEPCIONISTA", "PROFESIONAL", "PACIENTE"]}
              />
            }
          >
