...
export function AppointmentsPage() {
  const { data, user } = useClinic(); const [date, setDate] = useState(''); const [professional, setProfessional] = useState(''); const [specialty, setSpecialty] = useState(''); const [status, setStatus] = useState('')
  ...
  return <><PageHeading title={user?.role === 'PACIENTE' ? 'Mis citas' : 'Agenda'} description="Consulta y gestiona las reservas desde un solo lugar." action={user?.role !== 'PROFESIONAL' ? <Button asChild><Link to="/reservar">Nueva cita</Link></Button> : undefined} />
