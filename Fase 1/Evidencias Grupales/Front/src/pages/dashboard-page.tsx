import { CalendarCheck2, CalendarClock, CheckCircle2, CircleX, Stethoscope, Users } from 'lucide-react'
...
export function DashboardPage() {
  const { data, user } = useClinic(); if (!user) return null
  ...
  if (user.role === 'ADMIN') stats.push({ label: 'Profesionales', value: data.professionals.filter((p) => p.active).length, detail: 'Activos', icon: Stethoscope, className: 'bg-violet-50 text-violet-800' }, { label: 'Pacientes', value: data.patients.filter((p) => p.active).length, detail: 'Registrados', icon: Users, className: 'bg-amber-50 text-amber-800' })
  return <><PageHeading title="Inicio" action={user.role !== 'PROFESIONAL' ? <Button asChild><Link to="/reservar">Reservar una hora</Link></Button> : undefined} />
